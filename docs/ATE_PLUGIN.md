...
2. **New part in an existing family** -- `ate/config/parts/<key>.yaml` (`enabled_tests`, `vcc`, optional `vcc_sweep_list` or `vcc_sweep`, optional `vccb`, `fixture_modes`, `timing`). Create campaign folders under `#Test_Database/{Component}/{Part}/{Package}/{Operator}/Version_N/`. Or Setup **+ Version** for the next `Version_N`. Logic DC SKUs (2/3/N input): `docs/LOGIC_DC.md` -- truth table + pin roles + limits, not a forked runner.
3. **New test in an existing family** -- STANDARD FORMAT (same three live-UI paths as `docs/LOGIC_DC.md` + `AGENTS.md`): Path A `register(TestSpec)` then Setup **Test program** checkbox; Path B **Logic DC recipe** (same DC ids, yaml only); Path C Setup **Detected tests (golden + ate/tests)** -> **Wrap + enable on part**. Restart/reload worker. `pass_mode` is panel-editable. Checks: `check_add_test` / `check_family_load` / `check_logic_dc` / `check_test_detect`.
