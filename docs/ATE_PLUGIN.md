...
3. **New test in an existing family** -- preferred engineer path: `register(TestSpec(...))` in that family's package + add the id to the part's `enabled_tests`. Restart/reload worker. Run page shows a checkbox. Same three live-UI paths as `docs/LOGIC_DC.md`: Path A register, Path B product_model, Path C Detect/Wrap.
