...
5. `python -m ate.core.check_logic_dc` (also `check_add_test`, `check_family_load`). Visa-free Path B catalog: `python -m ate.core.check_logic_dc_sim` (the 24 CONFIRMED; overlay one VCC; `stable_eps_A` null = NON_TIGHT; G07 VOH SKIP; RS164 sequential 2^n SKIP). CONFIRMED Path B `TestSpec.run` walk (`_confirmed_sim_sweep_ok`) for GT34, G97, G126, G08, G07, G14, G32, GT08, GT32, G125, RS164 + scale-wave G00/G02/G04/G86/2G08/2G32 + GT-wave GT00/GT02/GT04/GT14/2G00/2G125/2GT08 -- `fixture_modes.SIM.tests` if present, else LOGIC `enabled_tests`. RS164 Path B gate ids stay OFF (sequential, not 2^n). Import-format alias `ate/tests/logic/dc.py` is the same runner (not a second fork). RS1G123 / RS1G74 are PARKED UNCONFIRMED archive -- not in the mandatory SIM set; missing yaml does not block green; FAIL if treated as gate 2^n. HOLD stubs RS1GT125 / RS1GT126 process-only (OE present, IOZ ABSENT -- do not invent IOZ). Scale-wave SoT signed vcc_grid / VOH / VOL / delta_icc_uA (JH DEMO DAY 2026-09-21 attached models). ICCT name ABSENT. excel_plots.status CONFIRMED for signed enabled series. Timeout SIM raises. **Not a reproduce claim.**
...
YAML `vcc_grid.status` CONFIRMED is Datasheet-signed (97/126/34, the 8 SoT SKUs: 08/07/14/32/GT08/GT32/G125/RS164, and G86 VIL fragment 2026-09-21). Overlay edits cannot promote unsigned SKUs. Unsigned `vcc_grid` on G00/G02/G04/2G08/2G32 stay UNCONFIRMED -- fail-closed for threshold numbers green.
...
**JH DEMO DAY CONFIRM-all scale wave (2026-09-21):** Attached SoT product_model CONFIRMED for RS1G00/G02/G04/G86/2G08/2G32 -- signed vcc_grid / VOH / VOL / delta_icc_uA. All six ICCT name ABSENT -- delta_icc from `delta_icc_uA.offset_v` 0.6 only (do not invent ICCT). RS1G00 NAND other=H invert; oe none; IOZ OFF; CMOS SoT. RS1G02 NOR other=L invert, TTL-style VIH/VIL (not G08 CMOS pair). RS1G04 INV n=1, NC not OE, IOZ OFF. RS1G86 XOR dual isolation track+invert; VIL 0.20*VCC + VIH 0.65*VCC at 1.65-1.95. RS2G08 / RS2G32 dual AND/OR with `recipe.dual_channel_continue` CHA then CHB; Path B A/B. Invent OE/IOZ/ICCT or skip CHA->CHB FAILs. excel_plots.status CONFIRMED (signed enabled series). LIVE START: `docs/LIVE_MATRIX_17.md` (Path B ticks) + `docs/LIVE_MATRIX_17_2026-09-21.md` (SoT 17 START narrative). No remote LIVE.

No Verify PASS.

## Ready vs Not ready (SIM only -- not bench green)

`python -m ate.core.check_logic_dc_sim` on the **24 CONFIRMED** Logic SKUs (skip OpAmp/LDO/Switch/Level). Visa-free. Fail-closed on invent / `stable_eps_A` null / glyph gaps. Dual-channel Continue path is ready. RS2G08 / RS2G32 signed VOH/VOL/DeltaICC + CHA then CHB. RS2G00 / RS2G125 / RS2GT08 unsigned VOH/VOL HOLD except 2G125 IOZ ON @3.6V UNSURE. Extra `rs2g*.yaml` without a Datasheet card stays forbidden. HOLD stubs RS1GT125 / RS1GT126 process-only.
...
| RS1G00 | SIM green | none | NAND other=H invert; ICCT ABSENT; signed CMOS/VOH/VOL/DeltaICC; excel_plots CONFIRMED; no invent OE/IOZ |
| RS1G02 | SIM green | none | NOR other=L invert; TTL-style VIH/VIL -- not G08 CMOS pair; signed loads |
| RS1G04 | SIM green | none | INV n=1; NC not OE; IOZ OFF; signed TTL bands + loads |
| RS1G86 | SIM green | none | XOR track+invert; VIL 0.20*VCC + VIH 0.65*VCC @1.65-1.95; excel_plots CONFIRMED |
| RS2G08 | SIM green | none | dual AND; CHA then CHB Continue; Path B A/B; signed loads; no invent OE/IOZ |
| RS2G32 | SIM green | none | dual OR; CHA then CHB Continue; Path B A/B; signed loads; no invent OE/IOZ |
| RS1GT00 | SIM green | none | TTL NAND; ICCT->dICC @3.4 not 0.6; unsigned VOH/VOL/VIH HOLD |
| RS1GT02 | SIM green | none | TTL NOR; not G08 CMOS; ICCT ABSENT |
| RS1GT04 | SIM green | none | TTL INV n=1; NC not OE; IOZ OFF |
| RS1GT14 | SIM green | none | Schmitt VT+/- ONLY; do not invent VIH/VIL; tick vth |
| RS2G00 | SIM green | none | dual NAND; CHA then CHB Continue; pin numbers HOLD |
| RS2G125 | SIM green | none | dual buf OE-L; IOZ ON @3.6V UNSURE; CHA then CHB; do not invent IOZ uA |
| RS2GT08 | SIM green | none | dual TTL AND; CHA then CHB; not G08 CMOS; ICCT ABSENT |
| RS1GT125 | HOLD (process-only) | none | OE present; IOZ ABSENT -- do not invent IOZ; VOH/VOL UNSURE; not SIM 24 |
| RS1GT126 | HOLD (process-only) | none | OE present active-H; IOZ ABSENT -- do not invent IOZ; VOH/VOL UNSURE; not SIM 24 |
...
| extra 2Gxx | Not ready | extra `rs2g*.yaml` without Datasheet card forbidden (named duals with cards: RS2G08/32/00/125 + RS2GT08) |
