...
RS2G08 / RS2G32 are **CONFIRMED** (JH DEMO DAY 2026-09-21) for grounded function/truth/isolation (`recipe.dual_channel_continue: true`, `channels: [CHA, CHB]`). Unsigned VOH/VOL/VIH and pin numbers HOLD. excel_plots.status UNCONFIRMED (enabled series only). Do not invent OE/IOZ. Extra `rs2g*.yaml` without a Datasheet card stays forbidden.
