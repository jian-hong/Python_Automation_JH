...
**SOOT correction.** JH last-day unlock **2026-09-21 Asia/Kuala_Lumpur** **CONFIRM-all** is **VOID**. JH DM unlock != Datasheet CONFIRM. Scale-wave G00/G02/G04/G86/2G08/2G32 on-disk cards stay **UNCONFIRMED DRAFT**. Function bind (NAND/NOR/INV/XOR/dual CHA then CHB, G86 VIL 0.20*VCC formula) stays. Numbers HOLD -- not greenable. PARKED is not scale. DEMO/SIM is not a reproduce / not Verify PASS.
...
3. **CONFIRM** -- JH Datasheet-signed `product_model.status: CONFIRMED` (and truth_table / isolation / signed `vcc_grid`). Until then **UNCONFIRMED** -- numbers HOLD, not greenable. Panel Save cannot promote signed. JH DM unlock is not this step.
...
CONFIRMED 11 + UNCONFIRMED DRAFT 6 + PARKED 2 listed below. G07 VOH N_A forever. IOZ only OE. RS164 != 2^n. Do not invent VOL live.

## Tomorrow without JH

1. GT34 **VOL board-change resume** (LIVE). Do not invent VOL. Keep VOH LIVE table as-is.
2. Scale-wave families stay **UNCONFIRMED DRAFT** (NAND/NOR/INV/XOR/dual AND/dual OR). Function bind only. Unsigned VOH/VOL/VIH (G86 VIL formula bind still UNCONFIRMED) stay **numbers HOLD**. Do not copy G08 CMOS onto G02. Do not invent ICCT. Do not LIVE-green until Datasheet CONFIRM.
...
### CONFIRMED (11) -- SIM mandatory; status gate only until LIVE
...
### UNCONFIRMED (6) -- scale-wave DRAFT; function bind; numbers HOLD

CONFIRM-all VOID. JH DM unlock != Datasheet CONFIRM. Not in mandatory SIM.

| Part | Function lock | SIM | Live / blocked |
|------|---------------|-----|----------------|
| RS1G00 | NAND other=H invert; oe none; IOZ OFF; ICCT ABSENT | UNCONFIRMED HOLD | unsigned VIH/VIL/VOH/VOL HOLD |
| RS1G02 | NOR other=L invert; TTL VIH/VIL (not G08 CMOS) | UNCONFIRMED HOLD | unsigned bands HOLD |
| RS1G04 | INV n=1 Y=NOT A; NC not OE; IOZ OFF | UNCONFIRMED HOLD | unsigned VIH/VIL/VOH/VOL HOLD |
| RS1G86 | XOR track+invert; VIL 0.20*VCC @1.65-1.95 formula bind | UNCONFIRMED HOLD | vcc_grid UNCONFIRMED; VIH/VOH/VOL HOLD |
| RS2G08 | dual AND; CHA then CHB Continue | UNCONFIRMED HOLD | pin numbers HOLD; unsigned loads HOLD |
| RS2G32 | dual OR; CHA then CHB Continue | UNCONFIRMED HOLD | pin numbers HOLD; unsigned loads HOLD |

`check_logic_dc` FAIL: invent OE/IOZ, invent ICCT, wrong G02/G86 bands, skip CHA->CHB, invent delta_icc numbers, invent VOH/VOL loads. Unsigned numbers gate FAIL/HOLD -- check PASSES while UNCONFIRMED.
...
## LIVE each family (CONFIRM-all VOID -- UNCONFIRMED DRAFT)

Shared start: START.bat, person selected, Open Session, Human Continue after wire_map, START not DEMO, null `stable_eps_A` = NON_TIGHT, fill golden_auto only, pretty never auto. Tick `input_threshold` + `icc` + `ii`. Do **not** tick `voh`/`vol`/`delta_icc`/`ioz` on these six until a Datasheet-signed card has loads / ICCT / OE. Do not claim Verify PASS / numbers green.
...
4. **LIVE XOR (RS1G86)** -- AWG A/B. Dual isolation track (other=L) + invert (other=H). Judge **VIL <= 0.20*VCC** at 1.65-1.95 (`max_only`) is function bind only -- vcc_grid UNCONFIRMED. VIH HOLD -- do not invent. Do not copy G08 VIL 0.15*VCC.
...
| G86 VIL | 0.20*VCC @1.65-1.95 function bind; vcc_grid UNCONFIRMED; VIH HOLD |
