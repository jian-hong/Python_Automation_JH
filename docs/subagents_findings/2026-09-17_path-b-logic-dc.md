...
- RS1GT34 DRAFT must stay UNCONFIRMED (do not call `_fail_closed_until_signed` on it -- that would FAIL check_logic_dc). Numbers fail-closed at run via greenable False -> unspec. Do not invent VOH/VOL/II/dICC/uA/PSU CH2. delta_icc without delta_offset_v FAIL-closes at run.
