...
11. Results `#btn-export-datalog` writes STS `sessions/datalog.md|.html|.pdf`. `#btn-fetch-datasheet` fills `ate/config/limits/<part>.yaml` from local Reference PDFs first (en.run-ic.com only if unfound). `#btn-fill-excel` writes `paste.values` numbers from living `report.json` (DUT list or CHA/CHB grid) for mapped/OpAmp books; Path B Logic DC overwrites the one Version xlsx (`workbook_policy: one_per_version_overwrite`) and never a second orphan book.
