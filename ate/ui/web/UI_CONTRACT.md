...
12. Setup `#add-test-format` (always visible on Test program) is the STANDARD FORMAT checklist (Path A Test program / Path B Logic DC recipe / Path C Detected tests -> Wrap + enable on part). `#panel-logic-dc` shows enabled tests, recipe (`logic_inputs`, `vcc_list`, `stable_eps_A`), per-field card editors from `docs/datasheet/card_fields.schema.yaml` (assign/edit/delete), truth table, isolation including PROPOSED/HOLD, ICC 2^n corners, and limits + `pass_mode`. Hidden when the part has no `product_model`. Test program `#test-list` `.test-pass-mode` is per-spec editable (range / min-only / max-only / fail-open / unspec). `#btn-save-pass-mode` and `#btn-save-test-params` write campaign `_manifest/test_params.yaml`. `#btn-save-logic-dc` writes part yaml via card_fields keys. UNCONFIRMED status is not greenable. Results table includes Mode. No extra tab.
